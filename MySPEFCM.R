My.spefcm=function(X, cluster, dimension, lambda.e, lambda.l, iterations, nstart, seed){
  ...
  for(ite in 1:ite.num){
    ...
    for(j in 1:(p*d)){
      ...
      if( (B.vec.dummy > 0 ) & ( abs(B.vec.dummy) > (lambda.l/2) )){
        B.vec[j] <- (B.vec.dummy - lambda.l/2)/sum(Z.mat1[,j]^2)
      }
      if( (B.vec.dummy < 0 ) & ( abs(B.vec.dummy) > (lambda.l/2) )){
        B.vec[j] <- (B.vec.dummy + lambda.l/2)/sum(Z.mat1[,j]^2)
      }
      if( ( abs(B.vec.dummy) <= (lambda.l/2) )){
        B.vec[j] <- 0
      }
      B <- matrix(B.vec,p,d)
    }
  ...
  return(list('U'=t(U), 'U2'=U2, 'OB'=OB_mat, 'X'=X %*% B, 'B'=B, 'G'=V %*% B %*% t(A), 'A'=A, 'cluster'=cluster.vec1))
}
